/* =====================================================================
   det-applets-alg.js · Tema 1 «Matrices»
   2.º de Bachillerato · Matemáticas Aplicadas a las Ciencias Sociales
   Ruta: 2-BatxMatesCCSS/matrices/assets/det-applets-alg.js

   CAPA DE ÁLGEBRA MATRICIAL del tema. Se carga DESPUÉS del núcleo
   det-applets.js y añade propiedades a window.DET. No registra ningún
...
(function () {
  ...
  if (!S) {
    if (window.console && console.warn) {
      console.warn('det-applets-lin.js necesita det-applets.js cargado antes.');
    }
    return;
  }
  ...
  function detPasos(M) {
    ...
    if (M.f === 3) {
      ...
      /* Los sumandos negativos SIEMPRE van entre paréntesis: se escribe
         «45 + 5 + (−40)», nunca «45 + 5 + −40». S.sumandosTex es el
         helper único del núcleo (det-applets.js) para esto, y
         S.parNegTex hace lo propio con un sumando aislado.          */
      var listaP = S.sumandosTex(vp.map(fTex));
      var listaN = S.sumandosTex(vn.map(fTex));
      tex = '\\begin{aligned}' +
        '\\det(A) &= ' + sp + ' - \\left(' + sn + '\\right) \\\\' +
        '&= \\left(' + listaP + '\\right) - \\left(' + listaN + '\\right) \\\\' +
        '&= ' + fTex(sumaP) + ' - ' + S.parNegTex(fTex(sumaN)) + ' = ' + fTex(v) +
        '\\end{aligned}';
      return {
  ...
  function filaPorColumna(fila, col) {
    ...
    return {
      ...
      /* Los productos negativos van entre paréntesis: «8 + (−5)» y no
         «8 + -5» (S.sumandosTex vive en el núcleo det-applets.js). */
