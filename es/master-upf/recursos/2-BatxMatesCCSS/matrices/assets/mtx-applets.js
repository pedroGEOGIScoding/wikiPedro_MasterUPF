/* =====================================================================
   mtx-applets.js · Tema 1 Matrices
   2.º de Bachillerato · Matemáticas Aplicadas a las Ciencias Sociales
   Ruta: 2-BatxMatesCCSS/matrices/assets/mtx-applets.js

   NÚCLEO del tema. Misma arquitectura que los motores de Números reales,
   Polinomios, Ecuaciones e inecuaciones, Sistemas de ecuaciones y los tres
   temas de Estadística de 2.º: un núcleo con las utilidades comunes y varios
   módulos que registran los applets.

   API pública: window.MTX
     ...
     ÁLGEBRA DE POLINOMIOS (coeficientes Frac, índice = grado). Se hereda de
     los temas anteriores y aquí se usa sobre todo para las entradas con
     parámetro: rangos y determinantes que dependen de k.
     ...
     .matricial             true cuando ha cargado mtx-applets-alg.js
     .extraA .extraB .extraC .extraD   true cuando cada módulo se ha cargado

   El álgebra de matrices propiamente dicha (dimensiones, operaciones,
   transposición, rango, Gauss, Gauss-Jordan, inversa y ecuaciones
   matriciales) vive en la capa mtx-applets-alg.js, que amplía este mismo
   objeto window.MTX. Toda la aritmética usa fracciones con BigInt, así que
   los pivotes de Gauss y los elementos de la inversa son exactos, sin
...
(function () {
  ...
  function shell(node, title, instr, fields, compute) {
    ...
    function run() {
      ...
      try {
        ...
        tex(out);
        (window.MTX && window.MTX.log ? window.MTX.log : []).push({ applet: title, error: e.message });
      }
    }
    api.run = run;
    api.ctl = ctl;
  ...
  R.diagnostico = function (node) {
    ...
    var filas = [
      ...
      ['Núcleo mtx-applets.js', true],
      ['Capa de álgebra matricial mtx-applets-alg.js', M.matricial === true],
      ['Módulo mtx-applets-a.js (matrices, tipos y transposición)', M.extraA === true],
      ['Módulo mtx-applets-b.js (operaciones y producto)', M.extraB === true],
      ['Módulo mtx-applets-c.js (combinaciones lineales y rango)', M.extraC === true],
      ['Módulo mtx-applets-d.js (inversa y ecuaciones matriciales)', M.extraD === true],
      ['Lectura de matrices desde texto', ok(function () {
        var A = m('1 2 3; 4 5 6');
        return A.f === 2 && A.c === 3 && A.a[1][2].val() === 6;
      })],
      ['Dimensión y elementos', ok(function () {
        var A = m('1 2; 3 4; 5 6');
        return M.dimTex(A) === '3 \\times 2' && A.a[2][0].val() === 5;
      })],
      ['Igualdad de matrices', ok(function () {
        return M.matIgual(m('1 2; 3 4'), m('1 2; 3 4')) === true &&
               M.matIgual(m('1 2; 3 4'), m('1 2; 3 5')) === false;
      })],
      ['Clasificación de matrices', ok(function () {
        var C = M.clasifica(m('1 0 0; 0 1 0; 0 0 1'));
        return C.identidad === true && C.diagonal === true && C.simetrica === true;
      })],
      ['Transposición y doble transposición', ok(function () {
        var A = m('1 2 3; 4 5 6');
        var T = M.matTrans(A);
        return T.f === 3 && T.c === 2 && M.matIgual(M.matTrans(T), A);
      })],
      ['Descomposición en simétrica y antisimétrica', ok(function () {
        var A = m('1 2 3; 4 5 6; 7 8 9');
        var D = M.descomponSim(A);
        return M.esSimetrica(D.S) && M.esAntisimetrica(D.H) &&
               M.matIgual(M.matSuma(D.S, D.H), A);
      })],
      ['Suma y producto por un número', ok(function () {
        var S = M.matSuma(m('1 2; 3 4'), m('5 6; 7 8'));
        var E = M.matEscalar(m('1 2; 3 4'), new M.Frac(3n));
        return S.a[0][0].val() === 6 && S.a[1][1].val() === 12 && E.a[1][0].val() === 9;
      })],
      ['Producto de matrices', ok(function () {
        var P = M.matProd(m('1 2 3; 4 5 6'), m('7 8; 9 10; 11 12'));
        return P.f === 2 && P.c === 2 && P.a[0][0].val() === 58 && P.a[1][1].val() === 154;
      })],
      ['El producto no es conmutativo', ok(function () {
        var A = m('1 1; 0 1'), B = m('1 0; 1 1');
        return !M.matIgual(M.matProd(A, B), M.matProd(B, A));
      })],
      ['Potencia de una matriz', ok(function () {
        var P = M.matPot(m('1 1; 0 1'), 5);
        return P.a[0][1].val() === 5;
      })],
      ['Rango por el método de Gauss', ok(function () {
        return M.rango(m('1 2 3; 2 4 6; 1 1 1')) === 2 && M.rango(m('1 0; 0 1')) === 2;
      })],
      ['Combinación lineal de filas', ok(function () {
        var C = M.combFilas(m('1 2 3; 4 5 6; 6 9 12'), 2);
        return C.dependiente === true;
      })],
      ['Inversa por Gauss-Jordan', ok(function () {
        var I = M.inversa(m('2 1; 1 1'));
        return I.existe === true && I.inv.a[0][0].val() === 1 && I.inv.a[0][1].val() === -1;
      })],
      ['Matriz singular sin inversa', ok(function () {
        return M.inversa(m('1 2; 2 4')).existe === false;
      })],
      ['Determinante exacto', ok(function () {
        return M.det(m('1 2 3; 4 5 6; 7 8 10')).val() === -3;
      })],
      ['Ecuaciones matriciales', ok(function () {
        var E = M.ecuMatricial('AX=B', m('2 1; 1 1'), m('3 0; 1 2'));
        return E.ok === true && E.X.a[0][0].val() === 2;
      })]
    ];
    /* --- Comprobación real de montaje de los paneles de la página ---
       ...
       nodos data-applet-mtx y se cuentan los que no han pintado nada.
       Se excluye el propio panel de diagnóstico, que en este instante
       todavía está vacío porque su innerHTML se asigna al final. */
    var vacios = [];
    var paneles = document.querySelectorAll('[data-applet-mtx]');
    Array.prototype.forEach.call(paneles, function (nd) {
      if (nd === node) return;
      if (nd.children.length === 0) vacios.push(nd.getAttribute('data-applet-mtx') || '(sin clave)');
    });
    ...
    var errs = window.MTX && window.MTX.log.length
      ? '<p class="ap-warn">Se han registrado ' + window.MTX.log.length + ' avisos: ' +
        esc(window.MTX.log.map(function (e) { return e.applet + ' — ' + e.error; }).join(' · ')) + '</p>'
  ...
  function monta() {
    ...
    Array.prototype.forEach.call(nodos, function (nd) {
      ...
      try {
        ...
        window.MTX.log.push({ applet: clave, error: String(e && e.message || e) });
